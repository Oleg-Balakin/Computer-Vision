...
cap = cv2.VideoCapture(0)   # 0 – первая веб-камера в системе, 1 – вторая и т.д.

while cap.isOpened():
    ret, frame = cap.read()
    ret, new_fraim = cap.read()
    ...
    if cv2.waitKey(10) == ord('q'):                    # открытое окно пока не нажата кнопка q
        break
